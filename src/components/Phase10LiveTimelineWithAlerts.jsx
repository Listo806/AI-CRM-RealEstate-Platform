// Ultimate MVP Frontend – Phase 10–14 (Frontend-Safe)
// ---------------------------------------------------
// React (Vite) + API endpoints only (no server imports)
// Copy/paste into Phase10LiveTimelineWithAlerts.jsx
...
function Timeline() {
  ...
  const fetchTasks = async () => {
    try {
      // Call server API (server handles MemoryVector, Asana fetch, etc.)
      const res = await axios.get('/api/fetchTimelineTasks');
      ...
      if(totalOverdue >= overdueThreshold){
        if(emailNotify) await axios.post('/api/sendOverdueEmail', {count: totalOverdue});
        if(slackNotify) await axios.post('/api/sendSlackAlert', {count: totalOverdue});
      }
...
// ------------------ Phase 11–14 Pages & Widgets ------------------
function LeadWidget() { /* same as before, API-only version */ }
function AnalyticsCMA() { /* same as before, API-only version */ }
function MarketingStudio() { /* same as before, API-only version */ }
function BrokerDashboard() { /* same as before, API-only version */ }
