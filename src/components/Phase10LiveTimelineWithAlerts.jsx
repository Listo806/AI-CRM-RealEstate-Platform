// Phase 10 - AI CRM Live Timeline with Alerts (Fully Preconfigured for Your Asana Project)
...
export default function Phase10LiveTimelineWithAlerts() {
  ...
  const overdueThreshold = 1; // Change if needed
  ...
  const fetchTasks = async () => {
    try {
      const res = await axios.get(`https://app.asana.com/api/1.0/projects/${boardId}/tasks`, {
        headers: { Authorization: `Bearer ${apiKey}` }
      });

      const tasks = res.data.data.map(task => ({
        day: parseInt(task.name.match(/Day (\d)/)?.[1] || 1) - 1,
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
      <div className="flex justify-center gap-4 mb-4">
        <label className="flex items-center gap-2"><input type="checkbox" checked={emailNotify} onChange={()=>setEmailNotify(!emailNotify)} /> Email Notifications</label>
        <label className="flex items-center gap-2"><input type="checkbox" checked={slackNotify} onChange={()=>setSlackNotify(!slackNotify)} /> Slack Notifications</label>
      </div>
      {showAlert && <div className="mb-4 p-4 bg-red-100 border-l-4 border-red-600 text-red-700 rounded shadow">⚠️ {overdueCount} tasks are overdue! Please review immediately.</div>}
